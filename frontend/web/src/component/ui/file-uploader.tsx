...
import {
  ...
  Pause,
  FileText,
  Image as ImageIcon, // ⬅️ alias pour l’icône
} from "lucide-react";
import NextImage from "next/image"; // ⬅️ next/image pour optimiser
...
export const FileTypeIcon: React.FC<{ type: string }> = ({ type }) => {
  if (type.includes("image")) {
    return <ImageIcon />; // ⬅️ l’icône lucide
  } else {
    return <FileText />;
  }
};
    ...
    >
      ...
      <div className="flex flex-col items-center justify-center space-y-3">
        ...
        <p className="text-sm text-black-500 opacity-80">
          Fichiers acceptés : .pdf, .docx, .doc, .png, .jpg, .jpeg
        </p>
      </div>
