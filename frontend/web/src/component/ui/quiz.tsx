...
const localQuiz: Question[] = [
  ...
  { id: 2, question: "What is the largest mammal in the world?", options: ["African Elephant", "Blue Whale", "Giraffe", "Polar Bear"], correctAnswer: 1 },
  { id: 3, question: "Which element has the chemical symbol 'Au'?", options: ["Silver", "Aluminum", "Gold", "Argon"], correctAnswer: 2 },
  { id: 4, question: "How many bones are there in an adult human body?", options: ["186", "206", "226", "246"], correctAnswer: 1 },
  { id: 5, question: "What is the speed of light in a vacuum?", options: ["299,792,458 m/s", "300,000,000 m/s", "186,000 miles/s", "All of the above"], correctAnswer: 0 },
  // Ajoute autant de questions que tu veux pour tester >5
  { id: 6, question: "The tallest mountain on Earth is?", options: ["K2", "Everest", "Kilimanjaro", "Denali"], correctAnswer: 1 },
  { id: 7, question: "H2O is the chemical formula for?", options: ["Hydrogen", "Oxygen", "Water", "Salt"], correctAnswer: 2 },
]

interface QuizService { loadQuiz(): Promise<Question[]> }
...
export function Component() {
  ...
  const calculateScore = (): number =>
    answers.reduce<number>((acc, ans, i) => (ans !== null && ans === questions[i].correctAnswer ? acc + 1 : acc), 0)
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="w-full max-w-3xl rounded-xl border border-slate-200 bg-white shadow-lg">
          <div className="text-center p-8 border-b border-slate-200">
            <div className="mb-4 flex justify-center"><Trophy className="w-16 h-16 text-sky-600" /></div>
          ...
          <div className="p-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {questions.map((q, i) => {
              ...
              return (
                <div key={q.id} className={`rounded-lg border px-3 py-2 text-sm text-center
          ...
          <div className="p-6 pt-0 text-center">
            <button
              onClick={() => { setQuizCompleted(false); setCurrentQuestion(0); setSelectedAnswer(null); setAnswers(Array(total).fill(null)) }}
      ...
      <div className="w-full max-w-3xl max-h-[calc(100vh-64px-2rem)] overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-lg flex flex-col">
        <div className="p-6">
          ...
          <div className="flex justify-between items-center mb-4">
            <div className="text-sm text-slate-600">Question {currentQuestion + 1} of {total}</div>
            ...
            {questions.map((_, i) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <div className={`text-[11px] ${isCurrent ? "text-sky-700 font-medium" : "text-slate-600"}`}>{i + 1}</div>
                </div>
              )
            })}
          </div>
