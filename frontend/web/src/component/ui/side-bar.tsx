...
const navigationItems: NavigationItem[] = [
    ...
    {id: "upload", name: "Uploader un document", icon: Upload, href: "/dashboard/upload"},
    {id: "resume", name: "Générer un résumé", icon: FileText, href: "/dashboard/upload-resume"},
    {id: "quiz", name: "Générer un quiz", icon: ClipboardList, href: "/dashboard/quizz"},
    {id: "chat", name: "Chatter avec l’IA", icon: MessageSquare, href: "/dashboard/chatter"},
];
...
export function Sidebar({className = ""}: SidebarProps) {
    ...
    return (
        <>
            ...
            <div
                className={`
                    bg-white border-r border-slate-200 z-40 transition-all duration-300 ease-in-out flex flex-col
            ...
            >
                ...
                <div className="flex items-center justify-between p-5 border-b border-slate-200 bg-slate-50/60">
                    {!isCollapsed && (
                        <div className="flex items-center space-x-2.5">
                            ...
                            <div className="flex flex-col">
                                <span className="font-semibold text-slate-800 text-base">StudyAI</span>
                            </div>
                        </div>
                    ...
                    {isCollapsed && (
                        <div
                            className="w-9 h-9 rounded-lg flex items-center justify-center mx-auto shadow-sm bg-[#3FA9D9]">
                ...
                <nav className="flex-1 px-3 py-2 overflow-y-auto">
                    ...
                    `}
                        ...
                        `}
                                        ...
                                        {!isCollapsed && (
                                            <div className="flex items-center justify-between w-full">
                                                <span
                                                    className={`text-sm ${isActive ? "font-medium" : "font-normal"}`}>{item.name}</span>
                                                {item.badge && (
                                                    <span className={`
                            px-1.5 py-0.5 text-xs font-medium rounded-full
                            ${isActive
                                                        ? "bg-blue-100 text-[#0A2540]]"
                        ...
                        <span className="text-[10px] font-medium text-[#0A2540]">
                          {parseInt(item.badge) > 9 ? '9+' : item.badge}
                                        ...
                                        {isCollapsed && (
                                            <div
                                                ...
                                                {item.badge && (
                                                    <span
                                                        className="ml-1.5 px-1 py-0.5 bg-slate-700 rounded-full text-[10px]">
                            {item.badge}
                          </span>
                                                )}
                                                <div
